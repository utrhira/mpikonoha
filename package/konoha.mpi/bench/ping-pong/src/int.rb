...
# *
    ...
    for n in 0 .. 18
        ...
        if b[0] != 1 or b[last] != (last + 1) then
            puts "ERROR - b[0] = "+ "%d" % b[0]+ " b[" + "%d" % last + "] = " + "%d" % b[last]
            exit(1)
        end

        for i in 1 .. (last-1)
            if b[i] != i then
                puts "ERROR - b[" + "%d" % i + "] = " + "%d" % b[i]
            end
        end
...
# *
    ...
    for n in 0 .. 18
        ...
        if b[0] != 1.0 or b[last] != (last + 1) then
            puts "ERROR - b[0] = " + "%d" % b[0] + " b[" + "%d" % last + "] = " + "%d" % b[last]
        end

        for i in 1 .. (last - 1)
            if b[i] != i.to_f then
                print "ERROR - b[" + "%d" % i + "] = " + "%d" % b[i]
            end
        end
        if myproc == 0 and time > 0.000001 then
            puts " " + "%7d" % size + " bytes took " + "%9.0f" % time + " usec (" + "%8.3f" % (2.0 * size / time) + " MB/sec)"
...
# *
    ...
    for n in 0 .. 18
        ...
        if a[0] != 1 or a[last] != (last + 1) then
            puts "ERROR - a[0] = " + "%d" % a[0] + " a[" + "%d" % last + "] = " + "%d" % a[last]
        end
        for i in 1 .. (last - 1)
            if a[i] != i then
                puts "ERROR - a[" + "%d" % i + "] = " + "%d" % a[i]
            end
        end
        if myproc == 0 and time > 0.000001 then
            puts " " + "%7d" % size + " bytes took " + "%9.0f" % time + " usec (" + "%8.3f" % (2.0 * size / time) + " MB/sec)"
