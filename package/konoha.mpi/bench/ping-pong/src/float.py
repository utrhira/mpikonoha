...
# *
    ...
    for n in xrange(18):
        ...
        a = numpy.zeros(size/8, 'd')
        b = numpy.zeros(size/8, 'd')
        ...
        if myproc == 0:
            ...
            world.Send([a, MPI.DOUBLE], other_proc)
            world.Recv([b, MPI.DOUBLE], other_proc)
            ...
            world.Send([b, MPI.DOUBLE], other_proc)

        t1 = MPI.Wtime()
        time = 1.e6 * (t1 - t0)
        world.Barrier()

        if b[0] != 1.0 or b[last] != float(last + 1):
...
# *
    ...
    for n in xrange(18):
        ...
        request = world.Irecv([b, MPI.DOUBLE], other_proc)
        world.Barrier()
        t0 = MPI.Wtime()
...
# *
    ...
    for n in xrange(18):
        ...
        request_b = world.Irecv([b, MPI.DOUBLE], other_proc)
        request_a = world.Irecv([a, MPI.DOUBLE], other_proc)
        ...
        if a[0] != 1.0 or a[last] != float(last + 1):
